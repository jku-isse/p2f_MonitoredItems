...
import at.pro2future.shopfloors.monitored.RestrictedAccessDelegate;
import at.pro2future.shopfloors.monitored.ValueLoggingDelegate;
...
public class Namespace extends ManagedNamespace {
    ...
    static final String NAMESPACE_URI = "at.pro2future.monitored";


    public UaNode getExpanded(ExpandedNodeId nodeId) {
    	return getNodeManager().get(nodeId, getNodeContext().getNamespaceTable());
    }
    
    public UaNodeManager getTheNodeManager() {
    	return getNodeManager();
    }
    
    public UaNodeContext getTheNodeContext() {
    	return getNodeContext();
    }
    
    public NodeFactory getTheNodeFactory() {
    	return getNodeFactory();
    }
    
    public NodeId getNewNodeId(String s) {
    	return newNodeId(s);
    }
    public NodeId getNewNodeId(int i) {
    	return newNodeId(i);
    }
